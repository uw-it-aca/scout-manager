...
class Command(BaseCommand):
    ...
    def addspotitems(self, workbook):
        ...
        spot_kane = client.getURL("/api/v1/spot/<spot_id>", headers)
        spot_kane = json.loads(spot_kane[1])
        spot_kane["items"] = []

        spot_health = client.getURL("/api/v1/spot/<spot_id>", headers)
        ...
        while sheet_types["A" + str(row)].value:
            if sheet_types["D" + str(row)].value == "UW Student":
                ...
                item["subcategory"] = self.subcategory_manager(
                    sheet_types["B" + str(row)].value
                )
                item["extended_info"] = {}
                item["extended_info"]["i_model"] = self.item_data_cleaner(
                ...
                item["extended_info"]["i_quantity"] = self.i_quantity_manager(
                    model, sheet_equipment, sheet_types, item_location
                )
                ...
                item = self.add_item_descriptions(
                    item, description_id, sheet_descriptions
                )
    ...
    def i_quantity_manager(
        self, model, sheet_equipment, sheet_types, item_location
    ):
        equipment_row = 2
        i_quantity = 0
        while sheet_equipment["A" + str(equipment_row)].value:
            if sheet_equipment["E" + str(equipment_row)].value == "UW Student":
                equipment_location = sheet_equipment[
                    "D" + str(equipment_row)
                ].value
                equipment_model = self.item_data_cleaner(
    ...
    def add_item_descriptions(self, item, descr_id, sheet_descriptions):
        ...
        while sheet_descriptions["A" + str(row)].value:
            if sheet_descriptions["A" + str(row)].value == descr_id:
                ...
                item["extended_info"]["i_reserve_url"] = str(
                    sheet_descriptions["D" + str(row)].value
                )
                if sheet_descriptions["E" + str(row)].value:
                    item["extended_info"]["i_manual_url"] = str(
